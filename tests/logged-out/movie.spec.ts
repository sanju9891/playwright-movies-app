...
test.describe('Movie Details Page - Content', () => {
  test(
    ...
    async ({ page }) => {
      ...
      await expect.soft(movie.getByLabel('Rating Value')).toHaveText('7.02');
...
test.describe('Movie Details Page - Links', () => {
  ...
  test('action genre link', async ({ page }) => {
    ...
    await expect.soft(page).toHaveURL(/Action/);
    await expect.soft(page.getByRole('heading', { level: 1 })).toHaveText('Action');
  });

  test('thriller genre link', async ({ page }) => {
    await page.getByRole('link', { name: 'Thriller' }).click();
    await expect.soft(page).toHaveURL(/Thriller/);
    await expect.soft(page.getByRole('heading', { level: 1 })).toHaveText(
      'Thriller',
    );
  });
  ...
  test('links to cast page and back button', async ({ page }) => {
    ...
    await expect
      .soft(page.getByRole('main').getByRole('heading', { level: 1 }).first())
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await expect
        .soft(
          page.frameLocator('iframe').getByRole('link', { name: 'Twisters' }),
        )
        .toBeVisible();
